...
- **Asymmetric Encryption (The Key Transporter):** Instead of using your asymmetric keypair to encrypt the data itself, you use it strictly to protect the small 32-byte transient AES key. Depending on the pattern, that protection is either a direct encryption of the key or a derived one-time key used to wrap it. Either way, this is known as Key Wrapping.
...
| ----------- | ------------------------------------- | ----------------- | ------------------------------------------- |
| doc_9081    | aesgcm:7b2a9f3c...0e4b                | user_alice_11     | wrapped:8a2b5f...c7e2 (Alice's self-wrap)   |
| doc_9081    | aesgcm:7b2a9f3c...0e4b                | user_bob_22       | wrapped:1e9d4c...b3a9 (Bob's access packet) |
| ...         | ...                                   | ...               | ...                                         |
...
| User ID       | KDF Parameters & Salt (Plaintext) | public_key (Plaintext)          | encrypted_private_key (Opaque Ciphertext) |
| ------------- | --------------------------------- | ------------------------------- | ----------------------------------------- |
...
| ----------- | ------------------------ | ----------------------------------------- | ----------------- | ------------------------------------------ |
| doc_9081    | f293a1e09923...          | aesgcm:7b2a9f3c...0e4b                    | user_alice_11     | wrapped:8a2b5f... (Alice's Packet)         |
| doc_9081    | f293a1e09923...          | aesgcm:7b2a9f3c...0e4b                    | user_bob_22       | wrapped:1e9d4c... (Bob's Packet)           |
...
- **The Key Exchange:** Alice generates a one-time ephemeral key pair and combines her ephemeral private key with Bob's static public key to derive the shared secret, the Key Encryption Key (KEK).
- **The Key Envelope:** Alice uses that newly calculated KEK to encrypt ("wrap") the 32-byte DEK.
- **The Transfer:** The temporary ephemeral public key exists solely to let Bob redo the same derivation and securely transport this DEK across the network.
...
Notice what made that convenience possible: one half of the shared secret, Alice's ephemeral public key, is stored forever, and the other half, Bob's private key, is deliberately long-lived so he can reach it at any time. That is the opposite of how Signal uses ECDH in a live chat, where both ephemeral halves are discarded the moment a session ends, leaving nothing durable behind to recompute the secret from. Trading that away is exactly what buys "decrypt it any time," and exactly what you give up in return is covered next.

## Why use it

Key Agreement is highly efficient, and when both sides' ephemeral keys are genuinely discarded after a live exchange, the way Signal's session handshakes work, it offers a critical security property known as Forward Secrecy: an attacker who compromises Bob's long-term private key next year still cannot retroactively decrypt past traffic, because the other half of the shared secret no longer exists anywhere. That is what makes ECDH the gold standard for real-time collaboration tools and modern sync protocols.

But the variant described above, where Alice's ephemeral public key is stored indefinitely alongside the document specifically so Bob can decrypt it later, is a different shape entirely: Bob's half of the secret is static by design, and Alice's half is preserved instead of discarded. That combination does not carry the Forward Secrecy property; it only looks like it does because it reuses the same ECDH math. Whether, and how, you can get Signal's actual guarantee in a store-and-forward system is covered next.
...
Key Agreement can solve this via Perfect Forward Secrecy, but only if the ephemeral material is truly thrown away after use. In a live exchange, each side generates a short-lived, throwaway ("ephemeral") curve key purely for that session; once the shared secret is calculated, the ephemeral private key is permanently deleted from memory, so a future compromise of a long-term static key alone can't reconstruct it.

**The catch for stored documents:** That guarantee depends on nothing being left behind that could reconstruct the shared secret later. Look back at "How to Decrypt the Data Later On": Bob recomputes the exact same KEK for a historical document using only his long-term static private key and Alice's ephemeral _public_ key, which travels with the document forever specifically so he can do this. That is precisely the calculation an attacker would run too. If Bob's static private key is ever compromised, every document he's ever received becomes retroactively decryptable, because the other half of the shared secret was never deleted, it was stored right next to the ciphertext. Forward Secrecy protects a transient exchange whose ephemeral material is genuinely discarded. It does not survive the moment you need that same exchange to be replayable for indefinite offline document storage, which is exactly what "decrypt it again next year" requires.
...
This is the real price of genuine Forward Secrecy: once that pre-key is deleted, Bob cannot re-derive the same shared secret again. If he wants to reopen the document later without weakening the guarantee, he has to have retained the resulting DEK (or the plaintext) from that first decryption, not the key material that produced it. That is a meaningfully different architecture from the "recompute it anytime with just your long-term key" convenience described earlier, and it is why systems that need both indefinite document access and true Forward Secrecy end up building real key-management infrastructure rather than a single static keypair.

<details>
<summary>If the keys get deleted, how does Signal show message history?</summary>
This raises an obvious question: if Forward Secrecy means the key material is genuinely gone, how does opening Signal show you a conversation from six months ago instead of a wall of undecryptable noise?

The answer is that Forward Secrecy only ever promised to protect the _key material_, not the _plaintext_. Signal's Double Ratchet actually goes further than the single ephemeral pair described above: it derives a brand-new message key for every individual message, and deletes each one the instant that message is decrypted. Nothing about that process is designed to be replayed later.

What makes history visible is that the decrypted plaintext is written to a local database on your device, protected by your device's own local storage encryption, entirely separate from the messaging protocol's key exchange. Scrolling through old messages is "read from a local database," not "redo the ECDH handshake." That is the same principle as the "retained DEK or plaintext, not the key material" distinction above, just applied to individual chat messages instead of stored documents.

This also reframes what Forward Secrecy is actually defending against. It protects a message in transit and against a future compromise of your long-term keys, an attacker who steals your identity key next year still can't reconstruct last year's message keys. It was never a promise that your conversation history would be unreadable to someone holding your unlocked phone; that is a local storage security problem, not a key agreement one.

A few consequences fall directly out of that shift:

- **Risk model moves to the endpoint:** the threat Forward Secrecy defeats, "can an attacker who steals a key read past traffic," is answered no, but the threat "can someone with physical or malware access to your unlocked device read your history" is answered yes, exactly like any other local storage. Signal's disk encryption and app lock exist to cover that gap; the key agreement protocol was never meant to.
- **Linking a new device replays nothing:** since the message keys genuinely no longer exist anywhere, a newly linked device cannot recompute old handshakes to backfill history. History has to be transferred as plaintext, device-to-device, at linking time.
- **Backups need their own key entirely:** restoring history on a fresh install also can't recompute anything, so it depends on a separately encrypted backup file protected by its own passphrase-derived key, not on the messaging protocol's keys at all.
- **Disappearing messages and "delete for everyone" become airtight:** once the plaintext copy is deleted from every device that held one, and no device ever retained the key material either, there is genuinely nothing left anywhere to reconstruct that message, by design rather than by hope.

</details>

Ultimately, choosing between RSA and ECDH is not just a math choice. It is an architectural design decision. For simple, offline-first document databases, Key Transport offers seamless usability out of the box, with the honest trade-off that a future key compromise exposes all history. For high-security environments, real-time sync engines, or apps requiring strict compliance, implementing a genuine one-time-prekey ECDH architecture, paired with a strategy for what happens to old documents after a prekey is consumed, is worth the engineering complexity to actually earn Forward Secrecy rather than just claim it.
...
async function encryptAndWrapForRecipientECDH(documentData: string, bobStaticPubJwk: string) {
  ...
  const aliceEphemeralKeyPair = await window.crypto.subtle.generateKey(
    { name: 'ECDH', namedCurve: 'P-256' },
    false, // Irrelevant to the public key (always exportable), but keeps the ephemeral private key non-extractable
    ['deriveKey'],
  );
...
- Choose Key Transport if your system is built around heavily asynchronous, one-way push states where data access permissions are modified completely independently by the document creator, and you accept that a future key compromise exposes all history.
- Choose Key Agreement if your priority is transient, real-time exchanges (live messaging, active sync sessions) where ephemeral material can genuinely be discarded after use, earning true Forward Secrecy. For indefinitely stored documents, that guarantee requires real key-management infrastructure (rotating one-time pre-keys, a plan for what happens after they're consumed), not just swapping RSA for ECDH.

By using standard Web Crypto APIs to pin non-extractable private keys into local device memory and using hybrid architecture patterns to isolate document payload data from key lifecycle states, you keep confidentiality in the hands of the users rather than the server, provided the public keys they exchanged were authentic in the first place. The keys stay exactly where they belong: securely in the hands of the users.
